...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    # 单机直接 python -m 启动,Ray 还没起,会进这个分支
    if not ray.is_initialized():
        # Initialize Ray with a local cluster configuration
        # Set environment variables in the runtime environment to control tokenizer parallelism,
        # NCCL debug level, VLLM logging level, and allow runtime LoRA updating
        # `num_cpus` specifies the number of CPU cores Ray can use, obtained from the configuration
        # 取 verl 默认要注入到 Ray worker 的环境变量字典(NCCL_DEBUG / VLLM_LOGGING_LEVEL 等),
        # 函数内部还会去重 driver shell 里已设过的 key,避免覆盖用户的 shell 值
        default_runtime_env = get_ppo_ray_runtime_env()
        # 取 yaml 里 ray_kwargs.ray_init 这一小块(给 ray.init() 用的参数,本次内容是 {num_cpus: null})
        # 注意:这里只读 ray_init 子节点,跟 data/actor/trainer 等命令行参数无关
        ray_init_kwargs = config.ray_kwargs.get("ray_init", {})
        # 用户没在 yaml 的 ray_init 下加 runtime_env,这里是 {}
        ...
        # 合并 verl 默认 runtime_env 与用户自定义(用户优先)
        runtime_env = OmegaConf.merge(default_runtime_env, runtime_env_kwargs)
        # 把合并后的 runtime_env 装回总参数字典 → 形如 {num_cpus: null, runtime_env: {...}}
        ray_init_kwargs = OmegaConf.create({**ray_init_kwargs, "runtime_env": runtime_env})
        print(f"ray init kwargs: {ray_init_kwargs}")
        # 真正启动 Ray 集群(单机模式):拉起 GCS / raylet / dashboard / worker pool
        # OmegaConf.to_container 把 OmegaConf 对象转成普通 dict,以便 ** 解包
        ray.init(**OmegaConf.to_container(ray_init_kwargs))

    # 把普通的 TaskRunner 类用 ray.remote 装饰器转成 Ray remote 类(每个实例占 1 个 CPU)
    # 转换后:不能直接 TaskRunner(),只能 TaskRunner.remote() 创建远程 actor
    ...
    if (
        ...
        # 在 Ray 集群里创建一个 TaskRunner actor 实例,返回 actor handle(不是真对象)
        runner = task_runner_class.remote()
    # 远程异步调用 TaskRunner.run(config) → 立刻返回 ObjectRef
    # ray.get 阻塞当前 driver 进程,等远程 run() 跑完;整个训练流程都在这一行内部执行
...
class TaskRunner:
    ...
    def __init__(self):
        # 多个role，比如ppo有ActorRolloutRef
        # 每个组worker进程数量=卡数量；组数量=1（actor和critic组合成dict），每张卡包括两种资源
        self.role_worker_mapping = {} # role->Worker Class
        self.mapping = {} # role->pool
