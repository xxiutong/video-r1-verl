...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        """
        self.resource_pool_manager.create_resource_pool() # pool:ResourcePool
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            if not class_dict:
                continue
            # 把几个Role组成一个Work Dict，这样的话一个组多个Work Dict，每个Work Dict所有的Role共享资源
            ...
            # 把worker_dict的连体方法拆分，外部看起来是两（3）个work group，但实际还是一个wg：self.actor_rollout_wg = spawn_wg["Role.ActorRolloutRef"] self.critic_wg = spawn_wg["Role.Critic"]
            spawn_wg = wg_dict.spawn(prefix_set=class_dict.keys())
            all_wg.update(spawn_wg)

        if self.use_critic:
            self.critic_wg = all_wg[str(Role.Critic)]
            self.critic_wg.reset() #加载模型，分片以及换logits头
            # assign critic loss
        ...
        self.actor_rollout_wg.init_model() #加载模型和优化器，分片以及启动vLLM
        ...
        # Support custom AgentLoopManager via config Actor计算复杂所以需要Manager
        ...
        # 用于把新权重从FSDP同步到vLLM,以及hybrid显存复用（FSDP和vLLM轮流占资源）
    ...
    def fit(self):
        ...
        self.checkpoint_manager.update_weights(self.global_steps) # actor同步给vLLM
        ...
        if self.config.actor_rollout_ref.rollout.skip.get("enable", False): #调试固定rollout用的
        ...
        self.max_steps_duration = 0  #ESI我用不上
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                if hasattr(self.actor_rollout_wg, "async_calls_finalize_fn_exec"): # 不阻塞上一step末尾的异步调用
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            old_log_prob, old_log_prob_mfu = self._compute_old_log_prob(batch) # 拼成一条没有padding的列表算
